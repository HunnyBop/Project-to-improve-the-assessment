...
@app.route('/extract')
def extract():
    return render_template("extract.html.jinja", error = "")
...
def product(product_id):
    ...
    with open(f"app/opinions/{product_id}.json", 'r', encoding="utf-8") as f:
        opinions_array = json.load(f)

    return render_template("product.html.jinja", product_id=product_id, stats=stats, opinions=opinions_array)
...
def opinions():
    ...
    while (url):
        ...
        if not response or not product_id:
            return render_template("extract.html.jinja", error = "Couldn't download this product :/")
